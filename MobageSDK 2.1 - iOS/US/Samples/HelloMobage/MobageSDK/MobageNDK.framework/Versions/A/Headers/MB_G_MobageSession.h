...
- (bool)login:(uint64_t) userId gamerTag:(NSString*) gamerTag;
- (NSString *)sessionId;
@end
...
@interface MB_G_MobageSession : NSObject<MB_G_MobageSession> 
{
	...
	BOOL triedWithProvidedCredentials;
	
	// The following are returned by the server when logging in.
	NSString *accessToken;
	NSString *accessSecret;
	
	NSString *oauth2Token;
	
	// These are initialised based on the data returned by the server when logging in.
	NSString *userID;
}
//@property (nonatomic, readonly, assign) BOOL isValid;
...
// We should privatize this initWithInfo method:
- (MB_G_MobageSession *)initWithInfo:(NSObject<MB_G_SessionEstablishmentInfo>*)establishmentInfo andWithAnalytics:(NSObject<MB_G_SessionAnalytics>*) analytics;
- (void)endSession;
