...
/**
 * Interface for purchasing virtual in-app items. To complete a purchase, your 
 * app typically follows these steps:
 * <ol>
 * <li>Call
 * <code>MBBankDebit::createTransactionForItem:quantity:comment:withCallbackQueue:onComplete:</code>, 
 * which creates a transaction and presents a purchase screen to the user. If the user completes the
 * purchase, the transaction's state changes from <code>kMBTransactionStateNew</code> to
 ...
 * capture the user's funds and change the transaction's state to
 ...
 * <a href="https://developer.mobage.com/en/resources/rest_api">Mobage REST API</a>. When a 
 * transaction is created through the REST API, its state is set to
 * <code>kMBTransactionStateNew</code>. Calling this method on the transaction does the following:
 * <ol>
 * <li>The transaction's state is set to <code>kMBTransactionStateAuthorized</code>.</li>
 * <li>The user's funds are placed into escrow.</li>
 * <li>The transaction's state is set to <code>kMBTransactionStateOpen</code>. The virtual item
 * should now be delivered to the user.</li>
 * </ol>
