...
#pragma mark Enums / Bitfields
	...
	/**
	 * MBCRemoteNotification_sendToUserIdCallback
	 * @description 
	 * @cb-param MBCSimpleAPIStatus status 			
	 * @cb-param MBCError * error 			AUTORELEASED
	 * @cb-param MBCRemoteNotificationResponse * response 			AUTORELEASED
	 */
	typedef void (*MBCRemoteNotification_sendToUserIdCallback)(
	 ...
	 * @param MBCString_Array *  extrasKeys The app-specific keys to include in the payload. Each key must be represented as a string. The following key names are reserved and cannot be used:<ul><li><c>badge</c></li><li><c>collapseKey</c></li><li><c>extras</c></li><li><c>iconUrl</c></li><li><c>message</c></li><li><c>sound</c></li><li><c>style</c></li></ul>			AUTORELEASED
	 * @param MBCString_Array *  extrasValues The app-specific values to include in the payload. Each value must be represented as a string.			AUTORELEASED
	 * @cb SendToUserOnCompleteCallback  onComplete 
	 * Send a remote notification to another Mobage user.
	 * The remote notification is queued on the Mobage server and may not be sent immediately. The
	 * maximum size of the notification's content is 256 bytes.
	 * <p>
	 * The notification can include app-specific key/value pairs that trigger a special behavior in
	...
	void MBCRemoteNotification_sendToUser(
										...
										const char *  style,
										const char *  iconUrl,
										MBCString_Array *  extrasKeys,
										MBCString_Array *  extrasValues,
